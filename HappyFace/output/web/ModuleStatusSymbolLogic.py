...
class ModuleStatusSymbolLogic(object):
	...
	function getModStatusSymbol($status,$mod_type)
	{
		...
		else if ($status == -2) { return '<img alt="" src="config/themes/""" + theme + """/mod_noinfo.png" />'; }
	    }
	...
	function getModNavSymbol($status,$mod_type,$mod_title,$mod_id)
	{
		...
		else if ($status == -2) { return '<img id="FNImage_'.$mod_id.'" class="HappyFNnavpic" alt="'.$html_title.'" title="'.$html_title.'" src="config/themes/""" + theme + """/nav_noinfo.png" />'; }
	    }
	    else if ($mod_type == "rated" || $mod_type == "unrated") {
